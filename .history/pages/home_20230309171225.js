    ...
    if(addCollectionComponentMusicProps.myProfile){
      try {
        ...
        let publicationsData = await Promise.all(myPublications.items.map(async publicationInfo => {
          ...
          if(publicationInfo.metadata.original && publicationInfo.metadata.original.url){
            publish.audioCover = 'http://lens.infura-ipfs.io/ipfs/'+publicationInfo.metadata.original.url.substring(7)
          }
    ...
    if(addCollectionComponentMusicProps.myProfile){
      try {
        ...
        let collectionsData = await Promise.all(myCollections.items.map(async collectionInfo => {
          ...
          if(collectionInfo.metadata.original && collectionInfo.metadata.original.url){
            collection.audioCover = 'http://lens.infura-ipfs.io/ipfs/'+collectionInfo.metadata.original.url.substring(7)
          }
