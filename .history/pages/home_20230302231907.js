  ...
  async function fetchPublications(index){
    ...
    if(addCollectionComponentMusicProps.myProfile){
      try {
        ...
        const userPublications = await client.query({
          query:publications,
          variables:{id:profileId,limit:12,publicationTypes:['POST','MIRROR'],cursor:dataRef.current.cursor[index],mainContentFocus:['AUDIO']}
        })
        let myPublications ={ ...userPublications.data.publications}
        console.log(myPublications)
        /******如果用户本次fetch的帖子小于12，说明用户没有更多帖子了，需要将fetch的开关关闭 */
  ...
  async function fetchCollections(index){
    ...
    if(addCollectionComponentMusicProps.myProfile){
      try {
        ...
        const userCollections = await client.query({
          query:collections,
          variables:{address:address,limit:12,publicationTypes:['POST'],cursor:dataRefCollections.current.cursor[index],mainContentFocus:['AUDIO']}
        })
        let myCollections ={ ...userCollections.data.publications}
        if(myCollections.items.length<12){
          console.log('用户没有更多收藏了')
        ...
        let collectionsData = await Promise.all(myCollections.items.map(async collectionInfo => {
          let collection = {}
          collection.id=collectionInfo.id
          collection.name =collectionInfo.audioName
...
function AddCollectionComponentMovie({addCollectionComponentProps}){
  ...
  useEffect(() =>{
    ...
    fetchCollections(0)
    //第一步首先装填收藏品并生成所含所有图片的数组，并保存至dataRefCollections.current.pictures
  },[addCollectionComponentProps.addCollectionComponentMovieFlag])
  /********************************装填出版物，获取个人在LENS服务器上的帖子，并生成数组****************************************/
  //这里的index用来表示，是重置出版物，还是获取下一个出版物，index 值为0或者1,0表示重置，重新装填一遍。1表示获取下次出版物并装填
  async function fetchPublications(index){
    console.log(addCollectionComponentProps.myProfile)
...
  return (
  <div className={styles.container}>
    <Header headerProps={{bodyACollectionFlag,bodyAExhibitionFlag,setbodyACollectionFlag,setbodyAExhibitionFlag,addCollectionComponentFlag,setaddCollectionComponentFlag,setaddCollectionComponentMovieFlag,setAddOrEditCollectionComponentMusicFlag,setbodyBCollectionBlankFlag,myProfile,setMyProfile,setMyBrochures,setMyBrochuresMusic,setMyBrochuresMovie,createUser,searchUser,setLoadingFlag}}></Header>
